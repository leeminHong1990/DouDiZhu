...
class iRoomOperation(object):
			...
			if room.room_type == const.AGENT_ROOM:
				if room.pay_mode == const.AGENT_PAY_MODE:
					...
					if switch.DEBUG_BASE:
						...
						# 开房失败应当需要退款
						utility.update_card_diamond(self.accountName, -card_cost, -diamond_cost, callback, "{} RoomID:{}".format(const.GAME_NAME, room.roomID))  # reason 必须为英文
	...
	def startGame(self, current_idx, tile_list, host_cards, swap_list):
		if getattr(self, 'client', None):
			self.client.startGame(current_idx, tile_list, host_cards, swap_list)

	def redeal(self, current_idx, tile_list, host_cards):
		if getattr(self, 'client', None):
			self.client.redeal(current_idx, tile_list, host_cards)

	def postOperation(self, idx, aid, data, next_idx):
		if getattr(self, 'client', None):
			self.client.postOperation(idx, aid, data, next_idx)
