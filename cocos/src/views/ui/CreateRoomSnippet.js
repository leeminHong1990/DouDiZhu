...
var CreateRoomSnippet = UISnippet.extend({
    initUI: function () {
        ...
        var group_parent_panel_list = [
            ...
            this.gamename_panel.getChildByName("max_mul_panel"),
            this.gamename_panel.getChildByName("pay_panel"),
        ];
        var group_chx_list = ["round_chx", "game_mode_chx","max_mul_chx", "pay_mode_chx"];
        var group_label_list = ["round_num_label_", "game_mode_label_","max_mul_label_", "pay_mode_label_"];
        var group_chx_num_list = [3 ,2 ,4, 2];
        var group_chx_func_list = [
            function (i) {
                //self.game_round = 10 * (i + 1);
                self.game_round = i>0 ? (i * 4):1;
            ...
            function (i) {
                switch (i){
                    case 0:
                        self.max_boom_times = 3;
                        break;
                    case 1:
                        self.max_boom_times = 4;
                        break;
                    case 2:
                        self.max_boom_times = 5;
                        break;
                    case 3:
                        self.max_boom_times = 9999;
                        break;
                    default:
                        break;
                }
        ...
        var single_chx_list = ["prepare_chx","second_chx"];
        var single_label_list = ["prepare_label","second_label_1"];
    ...
    initCreateInfo: function () {
        var default_info_json = '{"game_round":10, "pay_mode":0, "game_mode": 0 ,"hand_prepare":0 ,"op_seconds":1 ,"max_boom_times":3}';
        ...
        this.max_boom_times = info_dict["max_boom_times"] || 3;
    ...
    setCreateInfo: function () {
        ...
        attribute_list.push('"max_boom_times":' + this.max_boom_times.toString());
    ...
    updateCardDiamond: function () {
		...
		if (this.pay_mode === const_val.AA_PAY_MODE) {
			...
			if (this.room_type === const_val.CLUB_ROOM) {
				val = "楼主消耗 x " + (parseInt(this.game_round / 4) + 1);
			} else if (this.room_type === const_val.AGENT_ROOM) {
				val = "代理消耗 x " + (parseInt(this.game_round / 4) + 1);
			} else {
				val = "房主消耗 x " + (parseInt(this.game_round / 4) + 1);
			}
		}
