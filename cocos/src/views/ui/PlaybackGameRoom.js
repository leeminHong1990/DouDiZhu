...
var PlaybackGameRoom = cc.Node.extend({
	...
	_getPokerImgPath: function (card) {
		let rank = rules.get_rank(card);
		let suit = rules.get_suit(card);
		// 大小王
		if (rank > 20) {
			...
			return "Poker/pic_poker_small_" + rules.POKER_COLOR_DICT[suit] + "" + rank + '.png';
		}
	},
	...
	parseCommands: function (op_list) {
		for (var i = 0; i < op_list.length; i++) {
			let obj = op_list[i];
			let aid = obj[0];
			if (aid == const_val.OP_REDEAL) {
				this.commands = [];
			} else if (aid == const_val.OP_FIGHT_DEALER || aid == const_val.OP_BET || aid == const_val.OP_CONFIRM_DEALER) {
				continue;
			} else {
				let command = {};
				command.aid = aid;
	...
	doNext: function () {
		...
		if (commands.length > 0) {
			...
			let player = h1global.player();
			player.postOperation(command.serverSitNum, command.aid, command.tileList, command.nextServerSitNum);
